// v8.11 – Kompakte Ein-Blick-Tabelle + Entscheidungsbewertung
function ensureEffectiveComparisonTable(){
  ...
  wrap.innerHTML=`
    ...
    <div class="comparison-table-frame"><table class="comparison-table compact"><colgroup><col class="metric-col"><col class="value-col"><col class="value-col"><col class="diff-col"></colgroup><thead><tr><th>Wert</th><th>A</th><th>B</th><th>Δ</th></tr></thead><tbody id="effectiveTableBody"></tbody></table></div>
...
function ensureDecisionCard(){
  ...
  const summary=$('effectiveSummary');if(!summary)return;
  const d=document.createElement('div');d.id='decisionCard';d.className='decision-card';
  d.innerHTML='<div class="decision-eyebrow">WasBinIchWert-Bewertung</div><div id="decisionTitle" class="decision-title"></div><div id="decisionGrid" class="decision-grid"></div><div id="decisionText" class="decision-text"></div>';
  summary.before(d);
}
ensureEffectiveComparisonTable();ensureDecisionCard();
...
$('calcCompare').onclick=()=>{
  ...
  ensureEffectiveComparisonTable();ensureDecisionCard();$('tableLegendA').textContent=na;$('tableLegendB').textContent=nb;
  const dm=(bv,av)=>{const d=bv-av;return `${d>=0?'+':'−'}${eur(Math.abs(d),0)}`},dm2=(bv,av)=>{const d=bv-av;return `${d>=0?'+':'−'}${eur(Math.abs(d),2)}`},dh=(bv,av)=>{const d=bv-av;return `${d>=0?'+':'−'}${nf(Math.abs(d),0)}h`};
  ...
  const winner=b.effectiveNetHour>a.effectiveNetHour?nb:na, loser=winner===nb?na:nb;
  const yearDiff=b.effectiveAnnualNet-a.effectiveAnnualNet,timeDiff=b.effectiveHours-a.effectiveHours,hourDiff=b.effectiveNetHour-a.effectiveNetHour,netDiff=b.n-a.n;
  const winnerIsB=winner===nb;
  const moneyAdv=winnerIsB?yearDiff:-yearDiff,timeAdv=winnerIsB?-timeDiff:timeDiff,hourAdv=Math.abs(hourDiff);
  $('decisionTitle').textContent=`${winner} liegt insgesamt vorn`;
  $('decisionGrid').innerHTML=`<div><span>Eff. Vorteil</span><strong>+${eur(hourAdv)}</strong><small>je gebundener Std.</small></div><div><span>Jahreswert</span><strong>${moneyAdv>=0?'+':'−'}${eur(Math.abs(moneyAdv),0)}</strong><small>gegenüber ${loser}</small></div><div><span>Zeit</span><strong>${timeAdv>=0?'−':'+'}${nf(Math.abs(timeAdv),0)} h</strong><small>${timeAdv>=0?'weniger':'mehr'} gebunden/Jahr</small></div>`;
  let msg=`Unter den eingegebenen Faktoren liefert ${winner} den höheren Gegenwert pro tatsächlich gebundener Stunde.`;
  if(moneyAdv>100&&timeAdv>5)msg+=` Gleichzeitig bietet der Job einen höheren effektiven Jahreswert und benötigt weniger Zeit.`;
  else if(moneyAdv< -100&&timeAdv>5)msg+=` Dafür ist der effektive Jahreswert niedriger – du tauschst also Einkommen gegen mehr freie Zeit.`;
  else if(moneyAdv>100&&timeAdv< -5)msg+=` Dafür bindet er mehr Zeit – der finanzielle Vorteil wird also mit zusätzlichem Zeitaufwand erkauft.`;
  if(Math.abs(netDiff)>50){const monthlyWinner=netDiff>0?nb:na;if(monthlyWinner!==winner)msg+=` Auffällig: ${monthlyWinner} hat zwar das höhere Monatsnetto, gewinnt aber nicht beim effektiven Jobwert.`;}
  $('decisionText').textContent=msg;
};
