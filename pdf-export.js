// v8.26 – PDF-Bericht PRO: iOS teilen, Desktop direkt herunterladen
...
function downloadPdfBlob(blob,fileName){const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=fileName;a.style.display='none';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),30000)}
function isMobileApple(){return /iPhone|iPad|iPod/i.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1)}
async function createPdfReport(){
  ...
  try{
    ...
    textRight('Jobvergleich - PRO',right,y,9,bold,muted);textRight(new Intl.DateTimeFormat('de-DE').format(new Date()),right,y-14,8,font,muted);rule(y-27,gold,2);y-=58;
    ...
    const c0=left,c2=left+295,c3=left+412,c4=right,rowH=24;page.drawRectangle({x:left,y:y-rowH,width:right-left,height:rowH,color:rgb(.965,.975,.99)});text('Kennzahl',c0+4,y-16,8,bold,muted);textRight(na,c2-6,y-16,8,bold,muted);textRight(nb,c3-6,y-16,8,bold,muted);textRight('Delta',c4-4,y-16,8,bold,muted);y-=rowH;
    for(let i=0;i<rows.length;i++){if(y<150){page=pdf.addPage([595.28,841.89]);({width,height}=page.getSize());y=height-46;text('WasBinIchWert - Jobvergleich',left,y,12,bold,navy);y-=24}if(i>=11)page.drawRectangle({x:left,y:y-rowH,width:right-left,height:rowH,color:soft});const f=i>=11?bold:font,r=rows[i];text(r[0],c0+4,y-16,8,f,navy);textRight(r[1],c2-6,y-16,8,f,navy);textRight(r[2],c3-6,y-16,8,f,navy);textRight(r[3],c4-4,y-16,8,f,navy);rule(y-rowH);y-=rowH}
    ...
    if(isMobileApple()&&navigator.share&&navigator.canShare?.({files:[file]})){
      try{await navigator.share({files:[file],title:'WasBinIchWert Jobvergleich',text:`${na} vs. ${nb}`})}catch(shareError){if(shareError?.name!=='AbortError')downloadPdfBlob(blob,fileName)}
    }else downloadPdfBlob(blob,fileName);
